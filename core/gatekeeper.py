...
class Gatekeeper:
    ...
    def check_quality_gates(self, state_def: Dict[str, Any], outputs: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for gate in gates:
            ...
            # discover
            if "来源数 >= 1" in rule_str or "coverage" in gate_id and state_def.get('id') == 'discover':
            ...
            # analyze
            elif "每个来源至少提取 1 个实体" in rule_str or "entity_coverage" in gate_id:
            ...
            # verify
            elif "≥95% 的主张已验证" in rule_str or "verification_coverage" in gate_id:
            ...
            # synthesize
            elif "置信度变化 < 0.01" in rule_str or "confidence_converged" in gate_id:
