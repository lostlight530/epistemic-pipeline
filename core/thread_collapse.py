...
class ThreadCollapseEngine:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active": len([t for t in self._threads.values() if t.status == ThreadStatus.ACTIVE]),
            "collapsed": len([t for t in self._threads.values() if t.status == ThreadStatus.COLLAPSED]),
            "absorbed": len([t for t in self._threads.values() if t.status == ThreadStatus.ABSORBED]),
